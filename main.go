package main

import (
	"fmt"
	"os"
	"time"

	"github.com/gin-contrib/cors"
	"github.com/gin-gonic/gin"
	"github.com/remoteview/service-blocks/common"
	_ "github.com/remoteview/service-blocks/docs" // docs is generated by Swag CLI, you have to import it.
	"github.com/remoteview/service-blocks/routes"
	"github.com/swaggo/gin-swagger"
	"github.com/swaggo/gin-swagger/swaggerFiles"
)

// @title Blocks api
// @version 1.0
// @description Blocks api endpoint

// @contact.name Ivan Santos
// @contact.url https://ivansantos.me
// @contact.email pingme@ivansantos.me

// @host localhost:3000
// @BasePath /
func main() {
	goEnv := os.Getenv("GO_ENV")
	if goEnv == "" {
		goEnv = "development"
	}
	db := common.Init(goEnv)
	defer func() {
		err := db.Close()
		if err != nil {
			fmt.Println("Error closing server")
		}
	}()

	r := gin.New()

	r.GET("/_health", routes.HealthCheckHandler)
	r.GET("/blocks", routes.ListBlocksHandler)
	r.GET("/blocks/:id", routes.GetBlockHandler)
	r.POST("/blocks", routes.CreateBlockHandler)
	r.DELETE("/blocks/:id", routes.DeleteBlockHandler)

	r.Use(cors.New(cors.Config{
		AllowMethods:     []string{"GET", "POST", "PUT", "PATCH", "OPTIONS"},
		AllowHeaders:     []string{"Origin"},
		AllowAllOrigins:  true,
		ExposeHeaders:    []string{"Content-Length"},
		AllowCredentials: true,
		MaxAge:           12 * time.Hour,
	}))
	r.GET("/swagger/*any", ginSwagger.WrapHandler(swaggerFiles.Handler))

	fmt.Println("Running server!")
	err := r.Run(":3001")
	if err != nil {
		fmt.Println("Error starting server")
	}
}
